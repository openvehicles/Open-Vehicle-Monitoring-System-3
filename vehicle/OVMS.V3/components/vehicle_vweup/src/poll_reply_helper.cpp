...
float PollReplyHelper::FromUint8(const std::string &info, uint8_t bytesToSkip /*= 0*/)
{
    ...
    return static_cast<float>((uint8_t)Store[0 + bytesToSkip]);
}

float PollReplyHelper::FromUint16(const std::string &info, uint8_t bytesToSkip /*= 0*/)
{
    ...
    return static_cast<float>((((uint16_t)Store[0 + bytesToSkip]) << 8) + (uint16_t)Store[1 + bytesToSkip]);
}
